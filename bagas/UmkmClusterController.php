        ...
        $result = Cache::remember($cacheKey, 600, function () use ($request, $zoom, $gridSize, $minLat, $maxLat, $minLng, $maxLng) {
            ...
            if ($isUnfiltered && \Illuminate\Support\Facades\Schema::hasTable('umkm_grid_cluster')) {
                ...
                foreach ($gridData as $cell) {
                    ...
                    $clusters[$key]['lat_sum'] += $lat * $cell->jumlah_umkm;
                    $clusters[$key]['lng_sum'] += $lng * $cell->jumlah_umkm;
                    $clusters[$key]['count'] += $cell->jumlah_umkm;
                    $clusters[$key]['terverifikasi_count'] += $cell->terverifikasi_count;

                    if ($cell->kecamatan) {
                        $clusters[$key]['kecamatans'][$cell->kecamatan] = ($clusters[$key]['kecamatans'][$cell->kecamatan] ?? 0) + $cell->jumlah_umkm;
                    }
                }
            } else {
